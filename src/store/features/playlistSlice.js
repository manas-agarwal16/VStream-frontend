...
export const addToPlaylist = createAsyncThunk("addToPlaylist" , async (data) => {
    try {
        ...
        toast.error(error?.response?.data?.error || "error adding video to playlist");
    }
})

export const removeFromPlaylist = createAsyncThunk("removeFromPlaylist" , async (data) => {
    try {
        const res = await axiosInstance.delete(`/playlist/remove-from-playlist?title=${data.title}&video_id=${data.video_id}`);
        toast.success(`video removed from ${data.title} successfully`)
        return res.data;

    } catch (error) {
        toast.error(error?.response?.data?.error || "error removing video to playlist");
    }
})

export const deletePlaylist = createAsyncThunk("deletePlaylist" , async (data) => {
    try {
        const res = await axiosInstance.delete(`/playlist/delete-playlist?title=${data.title}`);
        return res.data;
    } catch (error) {
...
export const renamePlaylist = createAsyncThunk("renamePlaylist" , async (data) => {
    try {
        const res = await axiosInstance.patch(`/playlist/rename-playlist` , data);
        return res.data;
    } catch (error) {
        toast.error(error?.response?.data?.error || "error in renaming the playlist");
    }
})
